...
public class RemoveShiftFromOperationAction extends Action 
implements IWorkbenchAction, ISelectionListener
{
	...
	//CLASS
	///////////////////////////////////////////
	public static final String ID = "com.datagardens.nq.sdb.client.commands.production.removeShift";
	private static RemoveShiftFromOperationAction INSTANCE;
	
	public static RemoveShiftFromOperationAction generateAction(IWorkbenchWindow window)
	{
		INSTANCE = new RemoveShiftFromOperationAction(window);
		return getActionReference();
	}
	
	public static RemoveShiftFromOperationAction getActionReference()
	{
		return INSTANCE;
	}
	///////////////////////////////////////////
	...
	private RemoveShiftFromOperationAction(IWorkbenchWindow window) {
		this.window = window;
		setId(ID);
		setActionDefinitionId(ID);
		setText("Delete Shift");
		setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin(Client.PLUGIN_ID, 
				ImageKey.ICON_SHIFT_DELETE));
		this.window = window;
		...
		if(selectedNav instanceof ModelNavigationObject)
		{
			...
			if(navObj.getMainCategory() != null)
			{
				if(navObj.getSubCategory() == NavigationCategory.OPERATION)
				{
					...
					sheet.setSheetNumber(sheet.getSheetNumber() - 1);
					try
					{
					...
					catch (Exception e) 
					{
						MessageDialog.openError(Display.getCurrent().getActiveShell(),
								getText(),
								"Cannot remove shift: " + e.getMessage());
						e.printStackTrace();
					}
				}
